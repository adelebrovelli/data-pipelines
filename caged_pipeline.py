"""
Pipeline CAGED - Pressão Salarial por Ocupação e Município
=============================================================

Fluxo:
  1. Baixa o .7z de movimentações do Novo Caged direto do FTP do MTE
  2. Extrai o .txt de dentro do .7z
  3. Lê o .txt em chunks (o arquivo tem ~450MB / 4,3 milhões de linhas por mês)
  4. Trata em nível de PESSOA (uma linha por admissão/desligamento) -
     salários fora de um range plausível viram NULL em vez de a linha
     ser descartada, pra manter a contagem batendo com o oficial
  5. Sobe pro BigQuery já enriquecido com nome de ocupação/município/UF

A pressão salarial e o salário médio NÃO são calculados aqui - ficam
como campo calculado no Looker Studio, usando AVG() direto em cima do
salário individual (BigQuery ignora NULL automaticamente em AVG).

Uso:
    python caged_pipeline.py 202601        # mês específico (AAAAMM)
    python caged_pipeline.py               # mês mais recente disponível

Pré-requisitos:
    pip install pandas google-cloud-bigquery pyarrow py7zr --break-system-packages
"""
...
DIM_SECAO_PATH = Path(__file__).parent / "dim_secao.csv"
...
# de-para de seção CNAE para o Grande Grupamento oficial do Novo Caged/IBGE
...
GRANDE_GRUPAMENTO_PADRAO = "Serviços"  # seções H a U caem todas aqui
...
def agregar_mensal(caminho_txt: Path, tamanho_chunk: int = 500_000) -> pd.DataFrame:
    ...
    for i, chunk in enumerate(leitor):
        ...
        agg_total = (
            chunk.groupby(["competênciamov", "uf", "município", "seção", "tipo"])
        ...
        agg_salario = (
            chunk_valido.groupby(["competênciamov", "uf", "município", "seção", "tipo"])
            .agg(qtd_valido=("salário", "size"), soma_salario=("salário", "sum"))
            .reset_index()
        )

        agg = agg_total.merge(
            agg_salario, on=["competênciamov", "uf", "município", "seção", "tipo"], how="left"
        )
    ...
    bruto = (
        bruto.groupby(["competênciamov", "uf", "município", "seção", "tipo"])
...
def enriquecer_com_dims(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "secao" in df.columns:
        if DIM_SECAO_PATH.exists():
            ...
            print(f"[AVISO] {DIM_SECAO_PATH.name} não encontrado - descricao_secao ficará vazia")
            df["descricao_secao"] = None

        # Grande Grupamento oficial do Novo Caged/IBGE (5 categorias)
...
def preparar_dims_locais():
    ...
    secao = pd.read_excel(caminho_layout_estab, sheet_name="seção")
